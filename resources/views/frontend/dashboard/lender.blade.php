...
@section('title', 'Lending Dashboard - Campus Resource Exchange')

@section('content')

<div style="margin-bottom: 40px;">
    <h1 class="page-title">Lending Dashboard</h1>
    <p class="text-muted">Items you are lending to others</p>
</div>
    ...
    <div class="tab-pane fade show active" id="active" role="tabpanel">
                            ...
                            <div class="card-body">
                                ...
                                <div style="display: flex; gap: 15px; margin-bottom: 15px;">
                                    ...
                                    <div style="flex: 1;">
                                        ...
                                        <small class="text-muted">To: {{ $transaction->borrower->name }}</small>
                                        <br>
                                        <small class="text-muted">Lent: {{ $transaction->start_date->format('M d, Y') }}</small>
                                    </div>
                                </div>

                                <!-- Status -->
                                <div style="background: #f8f9fa; padding: 12px; border-radius: 8px; margin-bottom: 15px;">
                                    <small class="text-muted">Due: <strong>{{ $transaction->due_date->format('M d, Y') }}</strong></small>
                                    <p style="color: #0d6efd; font-weight: 600; margin: 5px 0;">
                                        @php
        ...
        @else
            <div style="text-align: center; padding: 40px;">
                ...
                <p class="text-muted">No active lending</p>
            </div>
        @endif
    </div>
    ...
    <div class="tab-pane fade" id="late" role="tabpanel">
        @if ($lateTransactions->count() > 0)
            <div class="alert alert-warning mb-4">
                <i class="bi bi-exclamation-circle"></i> {{ $lateTransactions->count() }} items overdue
            </div>
            <div class="row">
                @foreach ($lateTransactions as $transaction)
                    <div class="col-lg-6 mb-4">
                        <div class="card border-warning">
                            <div class="card-body">
                                <h5 style="color: #856404; font-weight: 600; margin-bottom: 10px;">
                                    {{ $transaction->item->title }}
                                </h5>
                                <p class="text-muted small mb-2">Borrowed by: {{ $transaction->borrower->name }}</p>
                                <p style="color: #856404; font-weight: 600;">
                                    <i class="bi bi-exclamation-triangle"></i>
                                    {{ now()->diffInDays($transaction->due_date) }} days overdue
                                </p>
                                <p class="text-muted small mb-3">Should be returned: {{ $transaction->due_date->format('M d, Y') }}</p>

                                <button type="button" class="btn btn-warning btn-sm w-100" data-bs-toggle="modal" data-bs-target="#reminderModal{{ $transaction->id }}">
                                    Send Reminder
                                </button>
                            </div>
                        </div>
    ...
    <div class="tab-pane fade" id="completed" role="tabpanel">
        ...
        @else
            <div style="text-align: center; padding: 40px;">
                ...
                <p class="text-muted">No completed lending</p>
            </div>
        @endif
    </div>
</div>
